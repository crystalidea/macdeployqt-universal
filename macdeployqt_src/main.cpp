...
int main(int argc, char **argv)
{
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        } else if (argument.startsWith(QByteArray("-qtdir"))) {
            LogDebug() << "Argument found:" << argument;
            int index = argument.indexOf('=');
            if (index == -1)
                LogError() << "Missing qt directory path";
            else
                customQtPath = argument.mid(index+1);
    ...
    if (customQtPath.isEmpty()) {
        LogError() << "Cannot proceed without Qt dir";
        return 1;
    }

    LogNormal() << "Qt deploy dir:" << customQtPath;
